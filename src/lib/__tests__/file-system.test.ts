...
test("getNode returns the node for an existing file", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "content");
...
test("getNode returns null for a non-existent path", () => {
  const fs = new VirtualFileSystem();
  expect(fs.getNode("/missing.txt")).toBeNull();
});
...
test("getNode normalizes the path before lookup", () => {
  ...
  expect(fs.getNode("folder/file.txt/")).not.toBeNull();
  expect(fs.getNode("//folder//file.txt")?.path).toBe("/folder/file.txt");
});

test("createDirectory does not auto-create missing parent directories", () => {
  const fs = new VirtualFileSystem();

  const dir = fs.createDirectory("/a/b/c");

  expect(dir).toBeNull();
  expect(fs.exists("/a")).toBe(false);
  expect(fs.exists("/a/b/c")).toBe(false);
});

test("createFile fails when a path segment is an existing file, not a directory", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "content");

  const file = fs.createFile("/test.txt/inner.txt", "nested");

  expect(file).toBeNull();
  expect(fs.exists("/test.txt/inner.txt")).toBe(false);
});

test("createDirectory fails when a path segment is an existing file, not a directory", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "content");

  const dir = fs.createDirectory("/test.txt/inner");

  expect(dir).toBeNull();
});

test("createFile treats an empty path as the root and refuses to overwrite it", () => {
  const fs = new VirtualFileSystem();

  const file = fs.createFile("", "content");

  expect(file).toBeNull();
  expect(fs.getNode("/")?.type).toBe("directory");
});

test("viewFile clamps the start of a view range below 1", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "line1\nline2\nline3");
  ...
  const negativeStart = fs.viewFile("/test.txt", [-5, 1]);
  expect(negativeStart).toBe("1\tline1");
});

test("viewFile clamps the end of a view range beyond the file length", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "line1\nline2\nline3");
...
test("replaceInFile escapes regex special characters in the search string", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/prices.txt", "price: $5.00 and $5x00");

  const result = fs.replaceInFile("/prices.txt", "$5.00", "$9.00");

  expect(result).toBe("Replaced 1 occurrence(s) of the string in /prices.txt");
  expect(fs.readFile("/prices.txt")).toBe("price: $9.00 and $5x00");
});

test("replaceInFile returns an error for an empty search string", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "hello world");

  const result = fs.replaceInFile("/test.txt", "", "x");

  expect(result).toBe('Error: String not found in file: ""');
  expect(fs.readFile("/test.txt")).toBe("hello world");
});

test("insertInFile returns an error when insertLine is undefined", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "line1\nline2");

  const result = fs.insertInFile(
    "/test.txt",
    undefined as unknown as number,
    "text"
  );

  expect(result).toBe("Error: Invalid line number: undefined. File has 2 lines.");
});
...
test("listDirectory on root returns an empty array for a fresh file system", () => {
  const fs = new VirtualFileSystem();
  expect(fs.listDirectory("/")).toEqual([]);
});

test("serialize on a fresh file system only contains the root", () => {
  const fs = new VirtualFileSystem();
  const serialized = fs.serialize();
...
test("reset clears all files and restores the empty root directory", () => {
  const fs = new VirtualFileSystem();
  fs.createDirectory("/src");
  fs.createFile("/src/index.ts", "content");
  fs.createFile("/README.md", "content");
  ...
  expect(fs.exists("/src")).toBe(false);
  expect(fs.exists("/src/index.ts")).toBe(false);
  expect(fs.exists("/README.md")).toBe(false);
  expect(fs.getNode("/")?.type).toBe("directory");
  expect(fs.listDirectory("/")).toEqual([]);
  expect(fs.getAllFiles().size).toBe(0);
});

test("reset allows creating files again after clearing", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/old.txt", "old");
  fs.reset();

  const file = fs.createFile("/new.txt", "new");

  expect(file).not.toBeNull();
  expect(fs.readFile("/new.txt")).toBe("new");
});

test("exported fileSystem singleton is a usable VirtualFileSystem instance", () => {
  expect(fileSystem).toBeInstanceOf(VirtualFileSystem);
  expect(fileSystem.getNode("/")?.type).toBe("directory");
});
